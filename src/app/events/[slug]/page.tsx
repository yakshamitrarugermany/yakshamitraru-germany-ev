...
const EVENTS: Record<string, EventDetail> = {
  "devi-mahatme-berlin": {
    ...
    related: [
      {
        slug: "ornament-and-origin-zurich",
    ...
    <section className="relative min-h-[92svh] w-full overflow-hidden bg-forest-deep text-cream">
      <div className="absolute inset-0">
        <Image
          ...
          priority
        />
        <div className="absolute inset-0 bg-linear-to-b from-forest-deep/80 via-forest-deep/40 to-forest-deep" />
        <div
          aria-hidden
          className="pointer-events-none absolute -top-40 right-[10%] h-150 w-150 gold-radial opacity-25"
        />
      </div>
      ...
      <div className="relative container-page pt-32 md:pt-40 pb-16 md:pb-24 min-h-[92svh] flex flex-col">
        <Link
          href="/events"
          className="inline-flex items-center gap-2 text-cream/70 hover:text-saffron text-sm w-fit"
        >
        ...
        <div className="mt-auto grid grid-cols-1 lg:grid-cols-[auto_1fr] gap-8 md:gap-12 items-end">
          <div className="hidden lg:grid place-items-center h-32 w-32 rounded-3xl bg-cream text-forest-deep shadow-2xl">
            <div className="text-center leading-none">
              <div className="font-mono text-xs uppercase text-crimson tracking-widest">
                {d.toLocaleString("en-GB", { month: "short" })}
              </div>
              <div className="font-serif text-6xl mt-2">{d.getDate()}</div>
              <div className="font-mono text-xs text-forest-deep/60 mt-2 tabular-nums">
                {d.getFullYear()}
              </div>
            </div>
          </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="min-w-0">
              ...
              <div className="mt-3 text-sm text-forest-deep/70 flex items-center gap-2">
                <Clock className="h-4 w-4 text-saffron" /> {event.doors} ·{" "}
                {event.duration}
              </div>
              <div className="mt-1 text-sm text-forest-deep/70">
                {event.language}
              </div>
            </div>
          </div>
            ...
            <div className="min-w-0">
              ...
              <div className="mt-5 flex flex-wrap items-center gap-3">
                <Button asChild variant="saffron" size="sm">
                  <Link href={event.ticketUrl}>
                    <Ticket className="h-4 w-4" /> Tickets · from{" "}
                    {event.ticketFrom}
                  </Link>
                </Button>
                <a
  ...
  return (
    <Section tone="muted" id="schedule">
      <SectionHeader
        ...
        title={
          <>
            Evening <span className="italic">unfolds</span>
          </>
        }
      ...
      <ol className="mt-12 md:mt-16 max-w-3xl">
        {items.map((s, i) => (
          <li
            key={s.time + s.label}
            className="grid grid-cols-[auto_1fr] gap-6 md:gap-10 py-6 border-t border-forest-deep/10 first:border-t-0"
          >
            <div className="font-mono text-lg md:text-xl tabular-nums text-saffron">
              {s.time}
            </div>
    ...
    <Section tone="cream" id="artists">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-8 mb-12 md:mb-16">
        <SectionHeader
          ...
          title={
            <>
              The <span className="italic">ensemble</span>
            </>
          }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
        {artists.map((a) => (
          ...
          >
            ...
            <div className="min-w-0">
              <div className="font-serif text-lg text-forest-deep truncate">
                {a.name}
              </div>
              <div className="text-xs eyebrow text-forest-deep/60 mt-1">
                {a.role}
              </div>
            </div>
          </BrandCard>
      ...
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-8 mb-12 md:mb-16">
        <div className="max-w-2xl">
          ...
          <h2 className="display-2 mt-5 text-cream">
            From <span className="italic text-saffron">rehearsal</span> to
            stage.
          </h2>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
        {images.map((img, i) => (
          ...
          >
            ...
            />
            <figcaption className="absolute inset-x-0 bottom-0 p-3 md:p-4 bg-linear-to-t from-forest-deep/85 to-transparent text-xs md:text-sm text-cream/90 font-serif italic">
              {img.caption}
            </figcaption>
          </figure>
        ))}
    ...
    <Section tone="muted" id="related">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-8 mb-12 md:mb-16">
        <SectionHeader
          ...
          title={
            <>
              More <span className="italic">nights</span>
            </>
          }
